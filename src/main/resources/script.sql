...
- CREATE TABLE movies (title VARCHAR(255) not null, runtime INTEGER not null,
  genre VARCHAR(255) not null, imdb_score DOUBLE not null, rating VARCHAR(255) not null);
...
- SELECT rating, AVG(imdb_score) AS average_score, MAX(imdb_score) AS maximum_score,
  MIN(imdb_score) AS minimum_score FROM movies GROUP BY rating;
- SELECT rating, AVG(imdb_score) AS average_score, MAX(imdb_score) AS maximum_score,
  MIN(imdb_score) AS minimum_score FROM movies GROUP BY rating HAVING COUNT(*) > 1;
